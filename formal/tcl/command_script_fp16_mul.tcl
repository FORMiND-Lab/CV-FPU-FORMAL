#============================================================================
# command_script_fp16_mul.tcl — FP16 MUL (op_i=3)
# Uses merged spec: fma_spec_wrap_fp16.cpp
#
# NOTE: The addend (C) operand is ignored by the spec in MUL mode
# (uses f16_mul(A,B)). The RTL's fpnew_fma forces C=0 internally.
# However, NaN/Inf on C can cause spurious mismatches due to different
# NaN propagation paths between f16_mul and fpnew_fma(MUL mode).
...
proc case_split_fp16 {} {
    ...
    # C_inf_NaN — commented out (same as original mul16.tcl).
    # C is constrained to non-special values in ual_main,
    # and ignored by both spec (f16_mul) and RTL (C forced to 0).
    #caseBegin C_inf_NaN
...
proc run_main {} {
    ...
    if { [proofstatus p] } {
        puts "=== FP16 MUL proof SUCCEEDED (${_elapsed}s) ==="
    } else {
        puts "=== FP16 MUL proof TIMED OUT or FAILED (${_elapsed}s) ==="
    }
}
