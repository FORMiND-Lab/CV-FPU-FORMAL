#============================================================================
# command_script_fp32_add_top.tcl — FP32 ADD (op_i=2, op_mod=0)
# Implementation: fpu_top_wrap_fp32 (fpnew_top with minimal config)
# Spec:           fma_spec_wrap_fp32.cpp (SoftFloat golden model)
#
# This is the first top-wrapper proof. It verifies that the fpnew_top-based
# implementation (with opgroup routing + arbitration) produces identical
# results to the SoftFloat spec for FP32 ADD.
#
# Differences from command_script_fp32_add.tcl (direct fpnew_fma):
#   - compile_impl uses fpu_top_wrap_fp32 instead of fma_hector_wrap
#   - Additional RTL files for fpnew_top hierarchy
#   - Otherwise identical spec / lemma / case-split structure
#============================================================================
...
proc compile_impl {} {
    create_design -name impl -top fpu_top_wrap_fp32 -clock clock -reset resetN -negReset
    vcs -sverilog \
        +incdir+../../../third_party/cvfpu/src/common_cells/include \
        ../../../third_party/cvfpu/src/fpnew_pkg.sv \
        ../../../rtl/fpu_top_formal_cfg_pkg.sv \
...
proc run_main {} {
    ...
    puts "=== Starting FP32 ADD (top wrapper) proof ==="
    ...
    if { [proofstatus p] } {
        puts "=== FP32 ADD (top wrapper) proof SUCCEEDED (${_elapsed}s) ==="
    } else {
        puts "=== FP32 ADD (top wrapper) proof TIMED OUT or FAILED (${_elapsed}s) ==="
    }
}
