...
set_hector_multiple_solve_scripts_list {orch_multipliers}
...
proc compile_spec {} {
    ...
    cppan -I../../third_party/softfloat/include \
          ...
          ../../third_party/softfloat/source/s_mulAddF16_hector.c \
          ../../third_party/softfloat/source/s_roundPackToF16_hector.c \
          ../../third_party/softfloat/source/s_normRoundPackToF16_hector.c \
          ../../third_party/softfloat/source/s_normSubnormalF16Sig_hector.c \
...
proc compile_impl {} {
    ...
    set_cutpoint fma_hector_wrap_fp16.i_fma.product
    vcs -sverilog \
...
proc ual_main {} {
    ...
    assume sigA_eq = (spec.hector_sigA(1) == impl.i_fma.mantissa_a(1))
    assume sigB_eq = (spec.hector_sigB(1) == impl.i_fma.mantissa_b(1))
    ...
    set_fml_var _hector_sym_strict_malformed_checks false
    set_hector_check_excess_shift false
    set_app_var hector_enable_hdps true

    # cutpoint: 切断乘法器输出，把11x11位乘法从验证路径中切开

    cutpoint impl_product = impl.i_fma.product(1)

    # 先证明乘法器正确性

    lemma mul_ok = impl.i_fma.product(1) == (impl.i_fma.mantissa_a(1) * impl.i_fma.mantissa_b(1))

    # 在此假设下验证后续逻辑

    assume mul_guarantee = (impl_product == (impl.i_fma.mantissa_a(1) * impl.i_fma.mantissa_b(1)))
