set _hector_comp_use_new_flow true
set _hector_softfloat_version custom
set _label "FP64 FMADD"
set_hector_sym_max_iterations 10
set _label "FP64 FMADD"
set_hector_multiple_solve_scripts true
set_hector_multiple_solve_scripts_list {orch_multipliers orch_custom_fma}

proc compile_spec {} {
    create_design -name spec -top hector_wrapper
    cppan -DSOFTFLOAT_FAST_INT64 -I../../third_party/softfloat/include \
          -I../../third_party/softfloat/source/RISCV \
          ../spec/fma_spec_fp64.cpp \
          ../../third_party/softfloat/source/f64_mulAdd.c \
          ../../third_party/softfloat/source/s_mulAddF64.c \
          ../../third_party/softfloat/source/s_roundPackToF64.c \
          ../../third_party/softfloat/source/s_normRoundPackToF64.c \
          ../../third_party/softfloat/source/s_normSubnormalF64Sig.c \
          ../../third_party/softfloat/source/s_shortShiftRightJam64.c \
          ../../third_party/softfloat/source/s_shiftRightJam32.c \
          ../../third_party/softfloat/source/s_shiftRightJam64.c \
          ../../third_party/softfloat/source/s_countLeadingZeros64.c \
          ../../third_party/softfloat/source/s_countLeadingZeros32.c \
          ../../third_party/softfloat/source/s_countLeadingZeros16.c \
          ../../third_party/softfloat/source/s_countLeadingZeros8.c \
          ../../third_party/softfloat/source/s_mul64To128.c \
          ../../third_party/softfloat/source/s_add128.c \
          ../../third_party/softfloat/source/s_sub128.c \
          ../../third_party/softfloat/source/s_shortShiftRightJam128.c \
          ../../third_party/softfloat/source/s_shiftRightJam128.c \
          ../../third_party/softfloat/source/s_shortShiftLeft128.c \
          ../../third_party/softfloat/source/RISCV/s_propagateNaNF64UI.c \
          ../../third_party/softfloat/source/RISCV/softfloat_raiseFlags.c \
          ../../third_party/softfloat/source/softfloat_state.c
    compile_design spec
}

proc compile_impl {} {
    create_design -name impl -top fma_hector_wrap_fp64 -clock clock -reset resetN -negReset
    set_cutpoint fma_hector_wrap_fp64.i_fma.product
    vcs -sverilog \
        +incdir+../../third_party/cvfpu/common_cells/include \
        ../../third_party/cvfpu/fpnew_pkg.sv \
        ../../third_party/cvfpu/fpnew_classifier.sv \
        ../../third_party/cvfpu/fpnew_rounding.sv \
        ../../third_party/cvfpu/fpnew_fma.sv \
        ../../third_party/cvfpu/common_cells/src/cf_math_pkg.sv \
        ../../third_party/cvfpu/common_cells/src/lzc_hector.sv \
        ../../third_party/cvfpu/common_cells/src/rr_arb_tree.sv \
        ../rtl/fma_hector_wrap_fp64.sv
    compile_design impl
}

proc ual_main {} {
    assume impl.go(1) == 1
    map_by_name -inputs -specphase 1 -implphase 1
    assume spec.rounding_mode(1) < 5
    assume impl.op_i(1) == 0
    assume impl.op_mod_i(1) == 0
    set_resource_limit 72000
    set_hector_check_array_bounds false
    set_hector_multiple_solve_scripts true
    set_hector_multiple_solve_scripts_list {orch_multipliers orch_custom_fma}
    set orch_solve_outputs_effort 2
    lemma result_eq = spec.result(1) == impl.result(1)
    lemma except_eq = spec.exceptions(1) == impl.exceptions(1)
}

proc case_split_fma64 {} {
    caseSplitStrategy basic

    caseBegin A_inf_NaN
    caseAssume (spec.multiplier(1)[62:52] == 11'h7ff)

    caseBegin B_inf_NaN
    caseAssume (spec.multiplicand(1)[62:52] == 11'h7ff)

    caseBegin C_inf_NaN
    caseAssume (spec.addend(1)[62:52] == 11'h7ff)

    caseBegin norm_norm_norm
    caseAssume (spec.multiplier(1)[62:52] != 11'h000)
    caseAssume (spec.multiplier(1)[62:52] != 11'h7ff)
    caseAssume (spec.multiplicand(1)[62:52] != 11'h000)
    caseAssume (spec.multiplicand(1)[62:52] != 11'h7ff)
    caseAssume (spec.addend(1)[62:52] != 11'h000)
    caseAssume (spec.addend(1)[62:52] != 11'h7ff)

    caseBegin A_dnorm
    caseAssume (spec.multiplier(1)[62:52] == 11'h000)
    caseAssume (spec.multiplicand(1)[62:52] != 11'h000)
    caseAssume (spec.multiplicand(1)[62:52] != 11'h7ff)
    caseAssume (spec.addend(1)[62:52] != 11'h000)
    caseAssume (spec.addend(1)[62:52] != 11'h7ff)
    caseEnumerate adn1 -expr spec.multiplier[51:0] -parent A_dnorm -type leading1

    caseBegin B_dnorm
    caseAssume (spec.multiplicand(1)[62:52] == 11'h000)
    caseAssume (spec.multiplier(1)[62:52] != 11'h000)
    caseAssume (spec.multiplier(1)[62:52] != 11'h7ff)
    caseAssume (spec.addend(1)[62:52] != 11'h000)
    caseAssume (spec.addend(1)[62:52] != 11'h7ff)
    caseEnumerate bdn1 -expr spec.multiplicand[51:0] -parent B_dnorm -type leading1

    caseBegin C_dnorm
    caseAssume (spec.addend(1)[62:52] == 11'h000)
    caseAssume (spec.multiplier(1)[62:52] != 11'h000)
    caseAssume (spec.multiplier(1)[62:52] != 11'h7ff)
    caseAssume (spec.multiplicand(1)[62:52] != 11'h000)
    caseAssume (spec.multiplicand(1)[62:52] != 11'h7ff)
    caseEnumerate cdn1 -expr spec.addend[51:0] -parent C_dnorm -type leading1

    caseBegin AB_dnorm
    caseAssume (spec.multiplier(1)[62:52] == 11'h000)
    caseAssume (spec.multiplicand(1)[62:52] == 11'h000)
    caseAssume (spec.addend(1)[62:52] != 11'h000)
    caseAssume (spec.addend(1)[62:52] != 11'h7ff)

    caseBegin BC_dnorm
    caseAssume (spec.multiplicand(1)[62:52] == 11'h000)
    caseAssume (spec.addend(1)[62:52] == 11'h000)
    caseAssume (spec.multiplier(1)[62:52] != 11'h000)
    caseAssume (spec.multiplier(1)[62:52] != 11'h7ff)
    caseEnumerate bcdn -expr spec.addend[51:0] -parent BC_dnorm -type leading1

    caseBegin AC_dnorm
    caseAssume (spec.multiplier(1)[62:52] == 11'h000)
    caseAssume (spec.addend(1)[62:52] == 11'h000)
    caseAssume (spec.multiplicand(1)[62:52] != 11'h000)
    caseAssume (spec.multiplicand(1)[62:52] != 11'h7ff)
    caseEnumerate acdn -expr spec.addend[51:0] -parent AC_dnorm -type leading1

    caseBegin ABC_dnorm
    caseAssume (spec.multiplier(1)[62:52] == 11'h000)
    caseAssume (spec.multiplicand(1)[62:52] == 11'h000)
    caseAssume (spec.addend(1)[62:52] == 11'h000)
    caseEnumerate abcdn -expr spec.multiplier[51:0] -parent ABC_dnorm -type leading1
}

proc make {} {
    compile_spec
    compile_impl
    compose
}

proc run_main {} {
    global _label
    set_host_file "host.qsub"
    set_user_assumes_lemmas_procedure "ual_main"
    set_hector_case_splitting_procedure "case_split_fma64"
    set_fml_var orch_distrib 16
    puts "=== Starting $_label proof ==="
    set t0 [clock seconds]
    solveNB p
    set t1 [clock seconds]
    puts "=== $_label proof complete ==="
    puts "elapsed: [expr {$t1-$t0}] seconds"
}
