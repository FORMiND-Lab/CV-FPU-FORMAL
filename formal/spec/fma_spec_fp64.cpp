...
// of the equivalence check. It wraps Berkeley SoftFloat's f64_mulAdd with
...
extern uint_fast16_t hector_sigA;
extern uint_fast16_t hector_sigB;
...
void hector_wrapper()
{
    ...
    uint64_t multiplier;         // FP32 operand A
    uint64_t multiplicand;       // FP32 operand B
    uint64_t addend;             // FP32 operand C
    uint64_t result;             // FP32 result

    float64_t f_multiplier;
    ...
    // ---- Input mapping: uint32_t → SoftFloat float64_t ----
