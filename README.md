...
│  DUT:    fma_wrap_fmad_fp32   │         │  Impl: fma_wrap_fmad_fp32.sv │
│          ← 统一复用 ──────────┼─────────┤                              │
└──────────────────────────────┘         └──────────────────────────────┘
              │                                               │
              ▼                                               ▼
...
│   └── fma_wrap_fmad_fp32.sv               # FP32 wrapper (go/valid 接口)
├── sim/
│   ├── tb/
...
│       ├── fma_dpi.cpp                     # DPI-C golden model 实现
...
│   │   └── fma_spec_wrap_fp32_fmadd.cpp     # FP32 spec (Hector API + SoftFloat)
│   ├── tcl/
...
│   │   ├── command_script_fp32_fmadd.tcl    # FP32: 完整证明 (case split)
│   │   ├── command_script_fma32_directed.tcl  # FP32: 快速冒烟 (11 cases)
│   │   ├── command_script_fma32_enu.tcl    # FP32: 枚举策略
...
| FP32 wrapper | `fma_wrap_fmad_fp32.sv` | — | `fma_wrap_fmad_fp32.sv` |
...
| Formal FP32 | FP32 全空间穷举等价证明 | — | ✅ 搭建完成 |
| Formal FP16 | FP16 全空间穷举 (FMA/ADD/SUB/MUL) | — | ✅ 搭建完成 |
| Corner | ±0, ±∞, NaN, sNaN, subnormal | ⏳ | ✅ case split 已覆盖 |
| Full Ops | FMSUB / FNMADD / FNMSUB (FP16) | ⏳ | ✅ 7 种运算全覆盖 |

## 核心模型

| 角色 | Cosim | Hector FP16 | Hector FP32 |
|------|-------|-------------|-------------|
| RTL (DUT) | `rtl/fma_wrap_fmad_fp32.sv` → `fpnew_fma` | `rtl/fma_wrap_fp16.sv` → `fpnew_fma` | `rtl/fma_wrap_fmad_fp32.sv` → `fpnew_fma` |
| Golden | `sim/csrc/fma_dpi.cpp` → SoftFloat `f32_mulAdd` | `formal/spec/fma_spec_wrap_fp16.cpp` → SoftFloat F16 ops | `formal/spec/fma_spec_wrap_fp32_fmadd.cpp` → SoftFloat `f32_mulAdd` |
